...
$encoded_id = $_GET['p'];
 ...
 $_SESSION['modtitle'] = $row['LTITLE'];
        ...
        if (isset($_FILES['pdfQuestion'])) {
            ...
            $target_dir1 = "C:/xampp/htdocs/htuwebapp/Files/Scheme/";
            ...
            if (file_exists($target_file1)) {
                ...
                if (move_uploaded_file($file1['tmp_name'], $target_file1)) {
                    ...
                    $sql = "INSERT INTO approvescheme (C_CODE, schemepath, Modid) VALUES (?, ?, ?)";
                    ...
                    if ($stmt) {
                        ...
                        if (mysqli_stmt_execute($stmt)) {
                            include 'reuploadschememail.php';
                            $sql_update = "UPDATE dissaprovescheme SET progress = 'Done' WHERE C_CODE = ?";
    ...
    $query = "SELECT s.*, c.C_TITLE, l.LNAME
    FROM dissaprovescheme s
    JOIN courses c ON s.C_CODE = c.C_CODE
    JOIN lecturer l ON c.LID = l.LID WHERE s.id = $course_id";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);
...
<head>
    ...
    <link rel="stylesheet" href="../css/edit.css?12345">
